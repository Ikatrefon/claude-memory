...
- **BLOKADA deployu:** wymaga decyzji Michała o (sub)domenie + dodania rekordu **DNS A → 195.35.56.37** (Hostinger; brak wildcard). Potem: build na VPS (amd64), kontener na nowym porcie, blok nginx (sprawdzić czy system nginx czy dockerowy workload_nginx — patrz [[infrastructure]], pamięć niespójna między sesjami → zweryfikować na VPS) + cert (certbot). PYTANIE OTWARTE: jaka subdomena (np. cv. / jobs. .mdmresearch.com) czy własna domena.
