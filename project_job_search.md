...
- `.job2-head { width:527pt }` + `svg{width:100%}` → skaluje każdy SVG do kolumny → daty wyrównane do prawej, loga do lewej. UWAGA: SVG-i mają różne viewBox (480–611) i różną wielkość logo w środku → **loga wychodzą różnej wielkości** (np. Publicis małe, Assembly/accenture duże). Jeśli przeszkadza — Michał ujednolica wielkość logo w Figmie. Daty w SVG bywają inne niż w cv.json (np. Assembly SVG „04.2025", cv.json „04.2024") — źródłem na str.2 jest SVG.
