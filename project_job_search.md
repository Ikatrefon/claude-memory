...
- **GOTCHA (ważne) — „duch" logo z Figmy:** Figma eksportuje logo jako `<rect fill="url(#pattern)">` (kafelek). **Chromium przy eksporcie do PDF źle renderuje `pattern`** → powiela kafelek (słaby drugi egzemplarz „ducha", np. nad accenture). FIX: zamienić wypełnienie pattern na bezpośredni `<image>` (ten sam data-URI, pozycja przeliczona z matrixa pattern→objectBoundingBox; `preserveAspectRatio="none"`). Zrobione dla `job_accenture.svg` (skrypt jednorazowy w pythonie). UWAGA: fix jest w pliku — **jeśli Michał re-eksportuje accenture, duch wróci** → ponowić fix. Zaproponowana (jeszcze nie zrobiona) automatyzacja: konwersja pattern→image przy każdym renderze w `pdf.py`.

## Strona 3 — PLAN / W TOKU (2026-06-29)
Struktura docelowa str.3 (ustalona z Michałem): **3 prace** (GEMACO, d-well, GREY) jak na str.2 (SVG głowa 527 + punkty 12pt pod spodem, podmieniane pod ofertę) · **edukacja** = statyczny SVG (BEZ modyfikacji) · **4 pionowe kolumny**: about me / my interests / tech skills / quotes (układ+font po stronie Claude). Zostają ozdobniki „…earlier" (góra) i „…and this is not over". **Podpis „Best regards / Michal Duchowski" USUNIĘTY** (zrobione).
- **PROBLEM OBJĘTOŚCI (uzgodniony):** str.3 gęsta — bazowe bullety: GEMACO 5, d-well **8**, GREY 4 (=17) + edukacja + 4 kolumny (about_me 15, interests 12, tech_skills 12, quotes 6). Przy sizingu jak str.2 NIE zmieści się. Michał ZGODZIŁ się **zredukować punkty w starych pracach** (Claude dobierze najważniejsze 2–3/pracę przy składaniu) + 4 kolumny mniejszym fontem (~10pt Arial Narrow). Dostroić po wgraniu SVG.
- **CZEKAM NA:** 4 SVG-i w `ELEMENTY/` (3 prace + edukacja). Stary układ str.3 (cols2 2-kolumnowy z experience[5:] + .edu + .about grid 4-kol) DO PRZEBUDOWY na nowy (analogicznie do str.2: `HEAD_SVG` rozszerzyć o GEMACO/d-well/GREY, blok `.job2`/punkty; education jako inline SVG; 4 kolumny .about zostają, font do zmniejszenia).
- **Dwa tory (ustalone):** wersja GRAFICZNA (ten szablon, dla człowieka) vs wersja ATS-friendly (jedna kolumna, czysty tekst, słowa kluczowe) — DO ZROBIENIA z tego samego cv.json.
